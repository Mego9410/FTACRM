...
export default async function DealProgressionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: deal }, { data: stages }, { data: events }, reasons, { data: owners }] =
    await Promise.all([
      supabase
        .from("deals")
        .select("id, status, target_completion_date, owner_id, created_at, agreed_price, buyer_solicitor_id, seller_solicitor_id, last_activity_at")
        .eq("id", id)
        .maybeSingle(),
